...
#### vRealize Operations
The Management Packs generated by the vROps Integration SDK will only run on vROps versions that supports containerized Management Packs. Currently, this is limited to on-premise vROps, version 8.7.2 or later.
In addition, at least one Cloud Proxy (also version 8.7.2 or later) must be set up in vROps, as containerized Management Packs must be run on a Cloud Proxy collector.

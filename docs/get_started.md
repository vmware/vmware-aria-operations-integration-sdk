...
to create, test, and install a simple Management Pack (integration) in VMware Cloud Foundation (VCF) Operations.
...
#### VCF Operations
The Management Packs generated by the VCF Operations Integration SDK will only run on versions that support containerized Management Packs. Currently, this is limited to on-prem installs, version 8.10 or later.
In addition, at least one Cloud Proxy (also version 8.10 or later) must be set up in VCF Operations, as containerized Management Packs must be run on a Cloud Proxy collector.
...
2. `Management Pack display name`
     ...
     The Management Pack display name will show up in VCF Operations (**Data Sources &rarr; Integrations &rarr;
...
6. `Enter a path to a EULA text file, or leave blank for no EULA`
     ...
     VCF Operations requires a EULA file to be present in a Management Pack. If one isn't provided, a stub EULA file (`eula.txt` in
...
7. `Enter a path to the Management Pack icon file, or leave blank for no icon`
     ...
     The icon is used in the VCF Operations UI if present. If it is not present, a default icon will be used. The icon file must be
...
prompts for each. This is similar to creating a new _Adapter Instance_ in the VCF Operations UI. Connections are automatically
...
* Endpoint URLs - This returns a list (possibly empty) of URLs that have distinct SSL certificates that VCF Operations can ask
  the end user to import into the TrustStore.
* Version - This returns
  the [VCF Operations Collector API](https://github.com/vmware/vmware-aria-operations-integration-sdk/blob/main/vmware_aria_operations_integration_sdk/api/vmware-aria-operations-collector-fwk2.json) version the
...
To install the `pak` file, in VCF Operations navigate to **Data Sources &rarr; Integrations &rarr;
Repository** and click `ADD`. Select and upload the generated `pak` file, accept the README, and install the management pack.

To configure the management pack, VCF Operations navigate to **Data Sources &rarr; Integrations &rarr;
